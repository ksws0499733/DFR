...
class RaiwaySegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {'image': img, 'label': clss, 'exist':  exist   ,'instens': inss, 'weight':weight}
    ...
    def data_augment(self,index):
        ...
        for i, index in enumerate(indices):
            ...
            #     _img,_target = random_shadow(_img,_target)
            ...
            if i == 0:  # top left
                ...
                tgt4 = np.full((s * 2, s * 2), 255, dtype=np.uint8)  # base image with 4 tiles
...
def random_shadow(img, targets, degrees=10, translate=.1, scale=.6, shear=10, border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # https://medium.com/uruvideo/dataset-augmentation-with-random-homographies-a8f4b44830d4
    # targets = [cls, xyxy]
