...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        if args.resume is not None:
            ...
            if not args.ft and 'optimizer' in checkpoint.keys():
                self.optimizer.load_state_dict(checkpoint['optimizer'])
            if 'best_pred' in checkpoint.keys():
    ...
    def training(self, epoch):
        ...
        for i, sample_list in enumerate(tbar):
            ...
            for _idx in range(1,len(sample_list)):
                # print('******',_idx)
                image, target, mask = sample_list[_idx]['image'], sample_list[_idx]['label'],sample_list[_idx-1]['mask']
                ...
                loss = self.criterion(output, target) if loss is None else (loss + self.criterion(output, target))
    ...
    def test(self, epoch):
        ...
        for i, sample_list in enumerate(tbar):
            assert isinstance(sample_list, list)
            for _idx in range(1,len(sample_list)):
                image, mask = sample_list[_idx]['image'], sample_list[_idx-1]['mask']
