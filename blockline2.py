...
# data =np.array(( 0xFECE1000000000C2, 0x1EA554BAA28B3440))
...
class railSegmenter:
    ...
    def run(self,img_batch, color = 1, canvas_batch=None):
        ...
        for id, img in enumerate(img_batch):
            ...
            # temp = np.zeros((img_batch.shape[2],img_batch.shape[3],3), dtype = 'uint8')
    ...
    def run2(self,img_batch, label_batch, color = 1, canvas_batch=None, is_draw = True):
        ...
        for id, (img, lable) in enumerate(zip(img_batch, label_batch)):
            ...
            bPoint_list = self.find_blockList(lable0, 16) #ballast
            
            rPoint_list = self.find_blockList(lable1, 16) #railway

            tPoint_list = self.find_blockList(lable2, 16) #track
            ...
            self.blines = self.updata2(bPoint_list)
            self.lines = self.updata2(rPoint_list)
            self.tlines = self.updata2(tPoint_list)
    ...
    def find_blockList(self, label, bin_number=16, axis=0):
        W = label.shape[1]
        # print("W",W)
...
def main_one_img(filename):
    ...
    if filename.endswith('_color.jpg'):
        ...
        mask_batch, draw_batch, block_batch, bd_batch = segmenter.run2(np.array([rawImage]),np.array([lablemask]), 255)#,np.array([canvas]))
...
def main_one():
    ...
    for file in file_list:
        if file.endswith('_color.jpg'):
            ...
            canvas = cv2.imread(imagePth0)
            rawImage = cv2.resize(canvas, (576, 416))
            # canvas = canvas.transpose((2,0,1))
            mask_batch, draw_batch, block_batch, bd_batch = segmenter.run2(np.array([rawImage]),np.array([lablemask]), 255)#,np.array([canvas]))
            for idx, (maks, draw, block, bd) in enumerate( zip(mask_batch,draw_batch,block_batch,bd_batch)):
                cv2.imshow('draw', draw)
...
if __name__ == "__main__":
    ...
    # main_one()
    # showTask()
    main_one_img(r'run/mycls5/CSPnet-A-S-ce-mycls5/output/000039_color.jpg')
