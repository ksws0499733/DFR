...
class OrderConvTD(nn.Module):
    def __init__(self, BatchNorm, input_dim, kernel_size):
        super(OrderConvTD, self).__init__()
  
        self.conRNN = ConvLSTM(input_dim,input_dim,kernel_size,1,batch_first=True,return_all_layers=True)

    def forward(self, x):
        b,c,h,w = x.size()
        x = F.interpolate(x,size=[20,w], mode='bilinear', align_corners=True)
...
class OrderConvDT(nn.Module):
    def __init__(self, BatchNorm, input_dim, kernel_size):
        super(OrderConvDT, self).__init__()
...
class OrderConvLR(nn.Module):
    def __init__(self, BatchNorm, input_dim, kernel_size):
        super(OrderConvLR, self).__init__()
    ...
    def forward(self, x):
        ...
        b,c,h,w = x.size()
        x = F.interpolate(x,size=[h,20], mode='bilinear', align_corners=True)
        x = torch.unsqueeze(x, dim=1) # B,1,C,H,W
        x = x.permute(0, 4, 2, 3, 1)  # B,W,C,H,1
        x = self.conRNN(x)
        x = x.permute(0, 4, 2, 3, 1)  # B,1,C,H,W
        x = torch.squeeze(x,dim=1)
        x = F.interpolate(x,size=[h,w], mode='bilinear', align_corners=True)

        return x

class OrderConvRL(nn.Module):
    def __init__(self, BatchNorm, input_dim, kernel_size):
        super(OrderConvRL, self).__init__()
...
class OrderConv(nn.Module):
    # def __init__(self, BatchNorm, input_dim, kernel_size,Order_directions=['TD','DT','LR','RL']):
    def __init__(self, BatchNorm, input_dim, kernel_size,Order_directions=['TD']):
        super(OrderConv, self).__init__()
        num_len = len(Order_directions)
        self.len = num_len
        # self.conRNN_list = []
        self.model = nn.Sequential()
        for i in range(num_len):
            if   Order_directions[i]=='TD':
                conRNN = OrderConvTD(BatchNorm,input_dim,(1,kernel_size))
            elif Order_directions[i]=='DT':
                conRNN = OrderConvDT(BatchNorm,input_dim,(1,kernel_size))
            elif Order_directions[i]=='LR':
                conRNN = OrderConvLR(BatchNorm,input_dim,(1,kernel_size))
            elif Order_directions[i]=='RL':
                conRNN = OrderConvRL(BatchNorm,input_dim,(1,kernel_size))
            else:
                raise Exception('wrong Order_directions.')
            self.model.add_module('order_'+Order_directions[i]+str(i),conRNN)
            # self.conRNN_list.append(conRNN)



    def forward(self, x):
        # for i in range(self.len):
        #     x = self.conRNN_list[i](x)       
        x = self.model(x)

        return x

class OrderConv2(nn.Module):
    def __init__(self, BatchNorm, input_dim, kernel_size, seq_len = 20):
        super(OrderConv2, self).__init__()
  
        self.conRNN01 = ConvLSTM(input_dim,input_dim,(1,kernel_size),1,batch_first=True,return_all_layers=True,seq_len = seq_len)
        self.conRNN02 = ConvLSTM(input_dim,input_dim,(1,kernel_size),1,batch_first=True,return_all_layers=True,seq_len = seq_len)
...
def build_orderConV(input_dim, kernel_size, BatchNorm):
    return OrderConv(BatchNorm,input_dim=input_dim,kernel_size=kernel_size)
