...
class Decoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm, attention = True):
        ...
        self.relu = nn.ReLU()
        self.last_conv1 = nn.Sequential(nn.Conv2d(input_dim+48, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
        ...
        self.last_conv2 = nn.Sequential(nn.Conv2d(input_dim+48, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
